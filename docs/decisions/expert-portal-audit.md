...
- `auth.users.phone = '821084342824'`, `phone_confirmed_at` 설정, `role=expert` 확인.
- `experts.phone = '+821084342824'` — 프로필 표시번호(`010-8434-2824`)와 로그인 번호 일치.
- 대시보드 데이터도 정상: 연결 기업 1건 · 지급 내역 2건 → 로그인 시 화면이 채워짐.

**⛔ 남은 1건 (사용자가 대시보드에서 직접 — MCP에 Auth 설정 도구 없음):**
1) Supabase 대시보드 → Authentication → **Sign In / Providers** → Phone → **Test OTP** 에 추가:
   `821084342824=123456`
   - ⚠️ 이 칸은 **`+` 없이** 국제형식 숫자만 (필드 안내문 명시). `+` 넣으면 거부됨.
   - ⚠️ 코드는 **정확히 6자리** (앱 검증 `lib/auth/schemas.ts:53` = `/^\d{6}$/`).
2) `/expert/login` 에서 `010-8434-2824` 입력 → 코드 `123456` → 대시보드 진입.
