...
export const ADMIN_SCOPE_DESCRIPTIONS: Record<AdminScope, string> = {
  settings:
    "‘설정 > 기업관리’에서 사업자 정보·개인정보 보호책임자·주소·업종과 프로젝트 분야 카테고리, 회사 로고를 바꿀 수 있습니다. 임직원 계정과 금액에는 접근하지 못합니다.",
  modules:
    "‘설정 > 기업관리 > 사용 기능’에서 전문가 섭외·전자결재·행사 운영 중 쓰지 않는 영역의 사용을 캐스트로그에 요청하고, 요청을 취소할 수 있습니다. 스스로 켤 수는 없습니다 — 계약 사항이라 캐스트로그가 승인합니다.",
  staff:
    "‘설정 > 임직원 설정’ 전체입니다. 가입 신청 승인, 직원 계정 직접 생성, 권한단계 변경, 비활성 처리, 직급 체계 관리에 더해 **기능별 권한 문턱·역할별 최소 레벨 조정까지 포함**합니다 — 대표가 올려 둔 통제를 이 위임자가 되돌릴 수 있으니 신중히 주세요. 대표 등급을 부여하는 것만은 대표만 가능합니다.",
  sending:
    "‘설정 > SMS 설정’에서 문자 공급자(솔라피 등) API 키와 발신번호를 등록·교체하고 테스트 발송을 할 수 있습니다. **키를 다루는 권한**이므로 문구만 고칠 사람에게는 아래 ‘발송 문구·템플릿’을 주세요.",
  templates:
