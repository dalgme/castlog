...
export const GRADE_DESCRIPTIONS: Record<UserGrade, string> = {
  ...
  team_lead: "배정 프로젝트에서 전 기능 실행 + 레벨 4에 없는 4가지 — 프로젝트 개설, 보유자료 일괄 등록, 섭외 긴급 취소(확정 후), 문자 자유 발송. PL은 이 레벨부터(기본값)",
  deputy: "배정 프로젝트에서 섭외요청·요청 회수(응답 전)·수락서·안내문자 등 실행. 개설·일괄 등록·긴급 취소(확정 후)·자유 문자는 불가. PM은 이 레벨부터(기본값)",
  senior: "배정 프로젝트에서 실무 입력 — 세션 계획, 후보 지정, 예정가, 첨부. 발송·상신은 위 레벨이 실행. 부PM은 이 레벨부터(기본값)",
