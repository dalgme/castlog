...
import { QUOTE_DOC_LABELS, type QuoteLogRow } from "@/lib/quotes/logs";

/**
 * 견적·내부실견적·정산 변경 로그 — 언제·누가·어떤 값을 바꿨는지, 어느 버전에서.
 * (기획 지시 2026-09-09: 이전 버전은 별도 저장하고 로그를 남긴다)
 */
export function QuoteLogsDialog({
  title,
  load,
}: {
  title: string;
  load: () => Promise<{ ok: true; rows: QuoteLogRow[] } | { ok: false; error: string }>;
}) {
  const [open, setOpen] = useState(false);
  const [pending, startTransition] = useTransition();
  const [rows, setRows] = useState<QuoteLogRow[] | null>(null);
    ...
    >
      ...
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            최근 300건. 버전·항목·이전 값·바뀐 값을 담당자별로 남깁니다.
          </DialogDescription>
        </DialogHeader>
            ...
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-white text-left text-muted-foreground">
                <tr>
                  ...
                  <th className="py-1 pr-2 font-medium">문서</th>
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="align-top">
                    ...
                    <td className="whitespace-nowrap py-1.5 pr-2">
                      {QUOTE_DOC_LABELS[r.docType]}
                      {r.version !== null && <span className="text-muted-foreground"> v{r.version}</span>}
                    </td>
                    <td className="whitespace-nowrap py-1.5 pr-2">{r.action}</td>
