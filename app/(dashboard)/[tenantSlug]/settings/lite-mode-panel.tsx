...
export function LiteModePanel({ enabled }: { enabled: boolean }) {
  ...
  const toggle = (next: boolean) => {
    const message = next
      ? "라이트 모드를 켭니다.\n\n전문가에게 나가는 발송(섭외요청 문자·이메일, 재안내, 수락서 송신)이 전부 중단되고, 비용·지급 메뉴가 닫힙니다. 섭외 확정은 전화 확인 후 '섭외 완료(수락서 생성)' 버튼으로 합니다.\n\n기존 데이터는 그대로 유지되며 언제든 다시 끌 수 있습니다."
      : "라이트 모드를 끕니다.\n\n섭외요청 발송·수락서 송신·비용·지급 기능이 다시 열립니다. 라이트 모드에서 기록한 섭외 현황은 그대로 이어집니다.";
  ...
  return (
    <div className="space-y-3">
      ...
      <ul className="space-y-1 text-xs leading-relaxed text-muted-foreground">
        ...
        <li>
          · <b>꺼지는 것</b> — 섭외요청 문자·이메일 발송, 재안내, 수락서 송신,
          비용·지급 메뉴.
        </li>
        <li>
