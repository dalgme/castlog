...
export default async function ProjectDetailPage({
    ...
    planPanel = {
      ...
      plans: gate.required
        ? gate.plans.map((v) => ({
            ...
            plannedAmountMax: v.plan.plannedAmountMax,
      ...
      currentPlannedAmountMax: snapshot.plannedAmountMax,
  ...
  const { data: positionRecords } = slotIds.length
    ? await supabase
        .from("engagement_slot_positions")
        .select(
          "id, slot_id, position_no, code, status, expert_id, engagement_id, assigned_expert_id, rank, expected_fee, expected_fee_max, unit_fee_online, unit_fee_offline, hourly_fee_online, hourly_fee_offline, fee_custom"
        )
        .in("slot_id", slotIds)
  ...
  const slotRows: SlotRow[] = slotRecords.map((s) => ({
    ...
    hourlyFeeOnline: s.hourly_fee_online,
    ...
    positions: (positionRecords ?? [])
      ...
      .map((p) => ({
        ...
        hourlyFeeOnline: p.hourly_fee_online,
        hourlyFeeOffline: p.hourly_fee_offline,
